...
FINAL_REPORT_FILE = 'final_workshop_report_v8.md'
...
def main():
    ...
    for area, data in FOCUS_AREAS.items():
        for file_name, text in summaries.items():
            if any(keyword in text.lower() for keyword in data['keywords']):
                insight = f"The article **{file_name.replace('.txt', '').replace('_', ' ').title()}** provides a detailed look at the **{area}**. \
    ...
    with open(FINAL_REPORT_FILE, 'w', encoding='utf-8') as report:
        report.write("# Final Workshop Report V8\n\n")
        ...
        for area, data in FOCUS_AREAS.items():
            ...
            if data['insights']:
                report.write("### Key Insights from Research\n\n")
