...
FINAL_REPORT_FILE = 'final_workshop_report_v4.md'
    ...
    for area, data in FOCUS_AREAS.items():
        for file_name, text in summaries.items():
            if any(keyword in text.lower() for keyword in data['keywords']):
                ...
                insight = f"The article **{file_name.replace('.txt', '').replace('_', ' ').title()}** provides valuable insights into the **{area}**. It highlights the importance of a **phased approach** to adopting agentic AI, starting with **small, well-defined PoCs** that deliver **measurable business value**. A key theme is the need for a **rich toolkit of AI/agent primitives** to enable the development of more complex and autonomous workflows over time. The article also touches upon the challenges of **enterprise buy-in** and the importance of a **strong business case** for any agentic AI initiative."
                data['insights'].append(insight)

    # --- Generate the final report ---
    with open(FINAL_REPORT_FILE, 'w', encoding='utf-8') as report:
        report.write("# Final Workshop Report V4\n\n")
        ...
        for area, data in FOCUS_AREAS.items():
            ...
            if data['insights']:
                ...
                for insight in set(data['insights']):
